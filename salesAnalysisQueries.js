...
const salesScore = `select *,        100 * profitable / (select sum(b.sale_price - b.sale_cost)
...
count(*) total 
from products, receipts, inventories, sales where recp_id = sale_recp_id and sale_inv_id = inv_id and inv_prod_id = prod_id
...
from (select staff_name, to_char(to_date(recp_timestamp, 'YYYY/MM/DD'), 'YYYY/MM') mon, sum(sale_price - sale_cost) - avg(staff_salary) profit_after_salary  from receipts, staffs, sales 
where recp_id = sale_recp_id and recp_staff_id = staff_id group by staff_name, mon order by mon asc) x) y group by mon
`;
// supplier scoring 
// scored based on on time, late, very late and business impacting; scaled as 1, 0.5, 1, 2; 5 + (a) - 0.5*b - c - d*2
const supplierScore = `select year||'/'||case when monthly < 9 then '0' else '' end || monthly, case when score > 10 then 10 when score < 0 then 0 else score end score
from (
select supl_company, date_part('year', to_date(devy_delivery_date, 'YYYY-MM-DD')) as year,
	date_part('month', to_date(devy_delivery_date, 'YYYY-MM-DD')) AS monthly, 
       10 - sum(case when devy_status = 'ON TIME' then 1 else 0 end) - 0.5 * sum(case when devy_status = 'LATE' then 1 else 0 end) - 0.5 * sum(case when devy_status = 'VERY LATE' then 1 else 0 end) - 2 * sum(case when devy_status = 'BUSINESS IMPACTING' then 1 else 0 end) score
from deliveries, suppliers where supl_id = devy_supl_id group by supl_company, year, monthly) x
`;

// sales profitability more important than amount of sales
// this is the query for the sales report against employees and time
// 
const salesProfit = `select staff_name, to_char(to_date(recp_timestamp, 'YYYY/MM/DD'), 'YYYY/MM') mon, sum(sale_price - sale_cost) - avg(staff_salary) profit_after_salary  from receipts, staffs, sales 
where recp_id = sale_recp_id and recp_staff_id = staff_id group by staff_name, mon order by mon asc
`;
// top best performing
const bestPerformer = `select prod_name, sum(sale_price - sale_cost) from products, receipts, inventories, sales where recp_id = sale_recp_id and sale_inv_id = inv_id and inv_prod_id = prod_id
group by prod_name order by 2 desc limit 10`;

// revenue comparison (month on month)
const monthlyProfit = `select to_char(to_date(recp_timestamp, 'YYYY/MM/DD'), 'YYYY/MM') mon, sum(sale_price - sale_cost) profit from receipts, sales where recp_id = sale_recp_id group by profit order by mon asc`;
// product type sales mix
const productType = `select cate_name, count(*) sold from product_categories, products, inventories, sales where cate_id = prod_cate_id and prod_id = inv_prod_id and inv_id = sale_inv_id group by cate_name
`;
// product type sales 
const productTypeProfit = `select cate_name, sum(sale_price - sale_cost) sales from product_categories, products, inventories, sales where cate_id = prod_cate_id and prod_id = inv_prod_id and inv_id = sale_inv_id group by cate_name
`;
// regional sales mix on map
const regionalProfit = `select round(recp_latitude, 3) latitude, round(recp_longitude, 3) longitude, sum(sale_price - sale_cost) profit from receipts, sales where recp_id = sale_recp_id group by latitude, longitude order by profit asc
`;
// supplier statuses 
// Monthly
...
from deliveries, suppliers where supl_id = devy_supl_id group by supl_company, year, monthly
`;
...
const weeklyProfit = `SELECT 	date_part('year', to_date(recp_timestamp, 'YYYY/MM/DD')) as year,
       ...
       sum(sale_price - sale_cost) profitable          
FROM receipts, sales
where sale_recp_id = recp_id
GROUP BY year, weekly
ORDER BY year, weekly
`;

const investments = `select * from investments`
